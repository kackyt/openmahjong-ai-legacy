========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : OpenMahjongClientDbg
========================================================================


AppWizard が生成した OpenMahjongClientDbg には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには OpenMahjongClientDbg を構成している各ファイルの概要説明が含まれています。

OpenMahjongClientDbg.dsp
    このファイル (プロジェクト ファイル) はプロジェクト レベルの情報を含み、
    シングル プロジェクトまたはサブ プロジェクトのビルドに使用されます。他の
    ユーザーとプロジェクト ファイル (.dsp) を共有できますが、メイク ファイル
    はローカルにエクスポートしてください。

OpenMahjongClientDbg.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また COpenMahjongClientDbgApp アプリケーションクラスを宣
    言します。

OpenMahjongClientDbg.cpp
    このファイルは COpenMahjongClientDbgApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。

OpenMahjongClientDbg.rc
...
res\OpenMahjongClientDbg.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル OpenMahjongClientDbg.rc によってインクルード
    されます。

res\OpenMahjongClientDbg.rc2
...
OpenMahjongClientDbgDlg.h, OpenMahjongClientDbgDlg.cpp - ダイアログ
    これらのファイルは COpenMahjongClientDbgDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な OpenMahjongClientDbg.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) OpenMahjongClientDbg.pch 
